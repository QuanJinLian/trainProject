...
<body>
	...
	<c:choose>
		<c:when test="${cancelyn =='y' }">
			<c:set var="yn" value="미처리"/>
		</c:when>
		<c:when test="${cancelyn =='p' }">
			<c:set var="yn" value="처리중"/>
		</c:when>
		<c:when test="${cancelyn =='c' }">
			<c:set var="yn" value="처리완료"/>
		</c:when>
	</c:choose>
	<table>	
			<tr >
				<th colspan="6" style="font-size: 30px; color: purple; text-align: left; border-bottom: 1px solid gray;">
				CancelList(${yn })
				</th>
				<th colspan="4" style="font-size: 15px; color: purple; text-align: right ; border-bottom: 1px solid gray; padding-top: 3em;">
					<a href="/trainProject/train/admin/OrderCancelList.do?cancelyn=y">
					미처리 조회
					</a>
						&nbsp;
					<a href="/trainProject/train/admin/OrderCancelList.do?cancelyn=p">
					처리중 조회
					</a>
						&nbsp;
					<a href="/trainProject/train/admin/OrderCancelList.do?cancelyn=c">
					처리완료 조회
					</a>
				</th>
			</tr>
			<tr   style="font-weight: 900; color: black;">
			<th  colspan="9" style="font-size: 8px; color: purple;">
			&nbsp;
			</th>
			</tr>
			<tr>
